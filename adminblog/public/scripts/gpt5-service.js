      ...
      if (!response.ok) {
        ...
        if (response.status === 403 && errorText.includes('dall-e-3')) {
          ...
          if (!response.ok) {
            ...
            // Check if no DALL-E models are available at all
            if (response.status === 403 && (fallbackErrorText.includes('dall-e-2') || fallbackErrorText.includes('model_not_found'))) {
              console.log('❌ No DALL-E models available - gracefully continuing without image generation');
              return {
                success: false,
                error: 'Génération d\'image indisponible - Votre plan OpenAI ne donne pas accès aux modèles DALL-E. L\'article a été créé sans image.',
