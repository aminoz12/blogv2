...
function loadApiConfig() {
  ...
  // Also check for the API key in the format expected by GPT5Service
  const openaiApiKey = localStorage.getItem('openai_api_key');
  if (openaiApiKey && (!apiConfig.gpt.apiKey || apiConfig.gpt.apiKey !== openaiApiKey)) {
    console.log('Found OpenAI API key in localStorage, syncing with config...');
    apiConfig.gpt.apiKey = openaiApiKey;
    saveApiConfig();
    updateApiFormFields();
  }
}
...
function setupEventListeners() {
  ...
  const testDalleApi = document.getElementById('testDalleApi');
  ...
  if (testDalleApi) {
    console.log('DALL-E test button found, adding event listener');
    testDalleApi.addEventListener('click', testDalleApiFunction);
  } else {
    console.error('DALL-E test button not found');
  }
...
function saveGptApiConfig() {
  ...
  // Also save the API key in the format expected by GPT5Service
...
async function testApiKey(apiKey) {
  try {
    ...
    if (response.ok) {
      ...
      showNotification('Clé API OpenAI validée avec succès !', 'success');
    } else {
      const errorText = await response.text();
      console.error('❌ API key is invalid:', errorText);
      showNotification('Clé API OpenAI invalide. Veuillez vérifier votre clé.', 'error');
    }
  } catch (error) {
    console.error('❌ Error testing API key:', error);
    showNotification('Erreur lors de la validation de la clé API', 'error');
  }
}

// Test DALL-E API specifically
async function testDalleApiFunction() {
  const apiKey = document.getElementById('gptApiKey').value.trim();
  
  if (!apiKey) {
    showNotification('Veuillez d\'abord saisir une clé API', 'error');
    return;
  }

  try {
    console.log('🧪 Testing DALL-E API...');
    showNotification('Test de DALL-E en cours...', 'info');
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.log('DALL-E-3 failed, trying DALL-E-2...', errorText);
    ...
    if (response.ok) {
      const data = await response.json();
      console.log('✅ DALL-E API test successful:', data);
      showNotification('DALL-E API fonctionne correctement !', 'success');
    } else {
      const errorText = await response.text();
      console.error('❌ DALL-E API test failed:', errorText);
      
      if (errorText.includes('dall-e-3') || errorText.includes('dall-e-2')) {
        showNotification('DALL-E non disponible sur votre plan OpenAI', 'error');
      } else if (errorText.includes('quota')) {
        showNotification('Quota API épuisé', 'error');
      } else {
        showNotification('Erreur DALL-E: ' + errorText, 'error');
      }
    }
  } catch (error) {
    console.error('❌ Error testing DALL-E API:', error);
    showNotification('Erreur lors du test DALL-E: ' + error.message, 'error');
  }
}
