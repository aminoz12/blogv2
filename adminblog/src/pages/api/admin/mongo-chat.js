...
export async function GET({ request }) {
  try {
    ...
    if (action === 'gpt5_auto_state') {
      // Get GPT5 auto-response state
      try {
        await initializeChatDatabase();
        ...
        const setting = await settingsCollection.findOne({ key: 'gpt5_auto_enabled' });
        
        return new Response(JSON.stringify({
          enabled: setting ? setting.value : true // default to enabled
        }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' }
        });
      } catch (error) {
        console.error('Error getting GPT5 auto state:', error);
  ...
  try {
    ...
    if (action === 'set_gpt5_auto_state') {
      ...
      try {
        ...
        console.error('Error setting GPT5 auto state:', error);
        return new Response(JSON.stringify({ 
          error: 'Failed to update GPT5 auto state',
          details: error.message 
        }), {
          status: 500,
          headers: { 'Content-Type': 'application/json' }
