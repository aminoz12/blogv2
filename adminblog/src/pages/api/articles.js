...
export async function GET({ request }) {
  try {
    ...
    // Parse featured_image JSON strings for each article
    const processedArticles = articles.map(article => {
      if (article.featured_image && typeof article.featured_image === 'string') {
        try {
          // Check if it's a JSON string (starts with { or [)
          if (article.featured_image.trim().startsWith('{') || article.featured_image.trim().startsWith('[')) {
            article.featured_image = JSON.parse(article.featured_image);
          } else {
            // It's a plain URL string, convert it to the expected object format
            console.log('🔧 Converting plain URL to object format for article', article.id, ':', article.featured_image);
          ...
          console.warn('⚠️ Could not parse featured_image JSON for article', article.id, ':', parseError.message);
          // If parsing fails, treat it as a plain URL and convert to object format
          article.featured_image = {
            url: article.featured_image,
            alt: article.title || 'Article image',
            type: 'url'
...
export async function POST({ request }) {
  try {
    ...
    if (featured_image) {
      try {
        ...
        console.log('🔍 Featured image details:', {
          ...
          hasServerUrl: typeof featured_image === 'object' && featured_image ? !!featured_image.serverUrl : false,
        ...
        // Ensure we have a valid URL for storage
        if (processedFeaturedImage && !processedFeaturedImage.url) {
          console.warn('⚠️ Processed image missing URL, skipping image storage');
    ...
    // Prepare featured image data for storage
    let featuredImageForStorage = null;
    if (processedFeaturedImage && processedFeaturedImage.url) {
      // Store as JSON string with all image metadata
      featuredImageForStorage = JSON.stringify(processedFeaturedImage);
      console.log('🖼️ Storing featured image as JSON:', featuredImageForStorage);
    } else {
      console.log('📁 No featured image to store');
    }
...
export async function PUT({ request }) {
  try {
    ...
    let query;
    let queryParams;
    let existingImageData = null; // Declare outside the blocks to fix scope issue
