...
export async function processFeaturedImage(imageData) {
  console.log('🔍 processFeaturedImage called with:', {
    ...
    hasServerUrl: typeof imageData === 'object' && imageData ? !!imageData.serverUrl : false,
  ...
  // If it's an object with serverUrl (from manual upload), process it
  if (typeof imageData === 'object' && imageData.serverUrl) {
    console.log('📁 Processing manual upload with serverUrl:', imageData.serverUrl);
    const result = {
      url: imageData.serverUrl,
      filename: imageData.filename || imageData.name || 'image.jpg',
      size: imageData.size || 0,
      type: imageData.type || 'image/jpeg',
      alt: imageData.alt || '',
      caption: imageData.caption || '',
      position: imageData.position || 'top'
    };
    console.log('✅ Manual upload processed:', result);
    return result;
  }
