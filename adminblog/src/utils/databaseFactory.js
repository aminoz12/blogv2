  ...
  try {
    ...
    if (query.includes('select') && query.includes('from articles')) {
      ...
      return articles.map(article => ({
        ...article,
        id: article.id || article._id, // Use integer id first, fallback to ObjectId
    ...
    if (query.includes('insert into articles')) {
      ...
      if (params && params.length >= 11) {
        const articleData = {
          ...
          featured_image: (() => {
            if (!params[7] || params[7] === 'null') return null;
            if (typeof params[7] === 'string') {
              // If it's a JSON string, parse and extract URL
              if (params[7].startsWith('{')) {
                try {
                  const parsed = JSON.parse(params[7]);
                  return parsed.url || null;
                } catch (e) {
                  return null;
                }
              }
              return params[7]; // Already a URL string
            }
            // If it's an object, extract the URL
            return params[7].url || null;
          })(),
        ...
        if (article) {
          ...
          featured_image: (() => {
            ...
            if (typeof article.featured_image === 'string') {
              // If it's already a string, check if it's a URL or JSON
              if (article.featured_image.startsWith('{')) {
                try {
                  const parsed = JSON.parse(article.featured_image);
                  return parsed.url || null; // Extract just the URL
                } catch (e) {
                  return null;
                }
              }
              return article.featured_image; // Already a URL string
            }
            // If it's an object, extract the URL
            return article.featured_image.url || null;
          })(),
    ...
    if (query.includes('delete from articles where id = ?')) {
      ...
      const articleIdParam = params[0];
      console.log('🔧 Raw article ID param:', articleIdParam, 'Type:', typeof articleIdParam);
      
      // Try both integer and string matching
      let result;
      
      // First try as integer (for new articles)
      if (!isNaN(articleIdParam)) {
        const intId = parseInt(articleIdParam);
        console.log('🔧 Trying integer ID:', intId);
        result = await collection.deleteOne({ id: intId });
        console.log('🔧 Integer delete result:', result.deletedCount);
      }
      
      // If no result, try as string (for ObjectId strings)
      if (!result || result.deletedCount === 0) {
        const stringId = String(articleIdParam);
        console.log('🔧 Trying string ID:', stringId);
        result = await collection.deleteOne({ id: stringId });
        console.log('🔧 String delete result:', result.deletedCount);
      }
      
      // If still no result, try as ObjectId in _id field
      if (!result || result.deletedCount === 0) {
        try {
          const objectId = new ObjectId(String(articleIdParam));
          console.log('🔧 Trying ObjectId:', objectId);
          result = await collection.deleteOne({ _id: objectId });
          console.log('🔧 ObjectId delete result:', result.deletedCount);
        } catch (objectIdError) {
          console.log('🔧 ObjectId creation failed:', objectIdError.message);
        }
      }
      
      console.log('✅ Final delete result - deleted count:', result.deletedCount);
    ...
    if (query.includes('update articles') && query.includes('status = \'deleted\'')) {
      ...
      // Keep ID as string for MongoDB - DON'T use parseInt!
      const articleId = String(params[params.length - 1]);
      console.log('🔧 Article ID for soft deletion (as string):', articleId);
